...
const postLogin = async (req, res, next) => {
    const invalid_credintials_message = 'Invalid credentials';
    try {
        const userFromDb = await selectUserByEmail(req.body.email);
        if (userFromDb.rows.length === 0) return next(new ApiError(invalid_credintials_message));

        const user = userFromDb.rows[0];
        if (!await compare(req.body.password, user.password)) return next(new ApiError(invalid_credintials_message,401));
