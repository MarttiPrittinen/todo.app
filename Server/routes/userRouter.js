    ...
    if (error) next(error)
        try {
            ...
            (error, result) => {
                ...
                return res.status(200).json({id: result.rows[0].id,email: result.rows[0].email,token: result.rows[0].token})
            }
        )
        } catch (error) {
            return next(error)
...
router.post('/login',(req, res, next) => {
    ...
    try {
        ...
        (error, result) => {
            if (error) next(error)
                if (!result) return next(new Error(invalid_message))
            if (result.rowsCount === 0) return next(new Error(invalid_message))
